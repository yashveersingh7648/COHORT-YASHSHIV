...
// GET all agencies or filtered list (UPDATED)
router.get("/", async (req, res) => {
  const { q } = req.query;
  ...
  const filter = q
    ? {
        $or: [
          ...
          { companyCity: { $regex: q, $options: "i" } },
          { companyPincode: { $regex: q, $options: "i" } },
          { companyEmail: { $regex: q, $options: "i" } }, // Added email search
          { userEmail: { $regex: q, $options: "i" } }    // Added userEmail search
        ],
      }
    : {};
  ...
  try {
    ...
    const dataWithUrls = results.map((item) => ({
      ...item,
      logoUrl: item.companyLogo
        ? `${process.env.BASE_URL || 'http://localhost:8000'}/uploads/${item.companyLogo}`
        : null,
      bannerUrl: item.companyBanner
        ? `${process.env.BASE_URL || 'http://localhost:8000'}/uploads/${item.companyBanner}`
        : null,
      pdfUrl: item.profilePdf
        ? `${process.env.BASE_URL || 'http://localhost:8000'}/uploads/${item.profilePdf}`
        : null,
    }));
...
// GET single agency by ID (UPDATED)
router.get("/:id", async (req, res) => {
  try {
    const agency = await FormDataModel.findById(req.params.id)
      .lean()
      .select('+companyEmail +userEmail'); // Force include emails
    ...
    agency.logoUrl = agency.companyLogo
      ...
      : null;
    agency.bannerUrl = agency.companyBanner
      ? `${process.env.BASE_URL || 'http://localhost:8000'}/uploads/${agency.companyBanner}`
      : null;
    agency.pdfUrl = agency.profilePdf
      ? `${process.env.BASE_URL || 'http://localhost:8000'}/uploads/${agency.profilePdf}`
      : null;
...
// GET agency by email (UPDATED)
router.get('/by-email/:email', async (req, res) => { // Changed route to avoid conflict
  try {
    const { email } = req.params;
    
    const agency = await FormDataModel.findOne({ 
      $or: [
        { companyEmail: email },
        { userEmail: email }
      ]
